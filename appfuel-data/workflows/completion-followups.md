...
Do not always suggest the same things. Do not suggest saving, canvases, reviews, or similar ads just because those tools exist. Choose based on what would materially improve the user's next decision.
...
- **User goal:** discovery, competitor research, creative inspiration, ad brief generation, review mining, saving/organizing, or visual synthesis.
- **Current artifact:** app list, selected app, paid ads, organic reels, reviews, a collection, a canvas, or empty/weak results.
- **Missing evidence:** app context, creative examples, review language, similar examples, organic counterpart, paid counterpart, rankings/revenue, or visual organization.
- **User momentum:** are they exploring broadly, choosing winners, preparing briefs, or asking for an artifact they can share?
- **Available identifiers:** app ids, ad `public_id`, reel `public_id`, `view_url`, collection id, canvas id.
...
If the app list is already narrowed and visual comparison would help:

- "Should I create a canvas with the top apps, revenue notes, and their strongest ad/reel examples?"
...
- "Should I group these by creator mechanic, UI moment, and hook type in a canvas?"
...
### Canvas Or Collection Exists

If a canvas exists:

- "Should I add these new findings to the existing canvas with grouped cards and HTML summary nodes?"
- "Should I snapshot the canvas so we can visually check the layout before sharing it?"

If a collection exists:

- "Should I add these findings to that collection or make a separate collection for this angle?"
- "Should I turn the collection into a canvas with pattern notes?"
...
Do not suggest saving or canvases when there is not enough evidence yet.
...
```text
Smart next moves: 1. pull reviews for the top app to turn praise/complaints into ad angles, 2. create a canvas with the best ad cards plus HTML pattern notes, 3. find similar ads for the strongest hook.
```
